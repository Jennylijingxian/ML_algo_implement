...
def load_docs(docs_dirname:str) -> Sequence[Sequence]:
    ...
    # for root, dirs, files in os.walk(docs_dirname):
    #     for file in files:
    #         if file.endswith(".txt"):
    #             with open(os.path.join(root, file), encoding='latin-1', mode='r') as f:
    #                 content = f.read()
    #                 wds = words(content)
    #                 docs.append(wds)
    # return docs
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        """
        doc_num, word_num = X.shape
        y_xor = np.logical_xor(y, 1)

        y = np.expand_dims(y, 0)            # (1, doc_num)
        y_xor = np.expand_dims(y_xor, 0).astype(int)    # (1, doc_num)

        label_1_word_cnt = np.matmul(y, X)  # (1, word_num)
        label_1_word_cnt = np.reshape(label_1_word_cnt, word_num) # vector of doc_num
        label_0_word_cnt = np.matmul(y_xor, X) #(1, word_num)
        label_0_word_cnt = np.reshape(label_0_word_cnt, word_num) # vector of doc_num

        label_1_total = np.sum(label_1_word_cnt) # vector of doc_num
        label_0_total = np.sum(label_0_word_cnt) # vector of doc_num

        self.p_w_1 = np.log((label_1_word_cnt + 1) / (label_1_total + word_num)) # vector of doc_num
        self.p_w_0 = np.log((label_0_word_cnt + 1) / (label_0_total + word_num)) # vector of doc_num

        self.p_1 = np.log(np.sum(y) / doc_num)
        self.p_0 = np.log(np.sum(y_xor) /doc_num)
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        """
        doc_num = X.shape[0]
        prob_1 = self.p_1 + np.matmul(X, np.expand_dims(self.p_w_1, axis=-1))  # (doc_num, 1)
        prob_0 = self.p_0 + np.matmul(X, np.expand_dims(self.p_w_0, axis=-1))  # (doc_num, 1)
        res = (prob_1 > prob_0).astype(int)
        res = np.reshape(res, doc_num)
        return res
...
def kfold_CV(model, X:np.ndarray, y:np.ndarray, k=4) -> np.ndarray:
    ...
    interval = math.ceil(data_num / k)
    accuracies = []
    for i in range(0, data_num, interval):
        begin = i
        end = i + interval
        if end > data_num:
            end = data_num
        ...
        third = range(end, data_num)
        # first = range(begin)
        # second = range(begin, end)
        # third = range(end, data_num)
        train_idx = list(first) + list(third)
        test_idx = list(second)
        train_x = X[train_idx, :]
        test_x = X[test_idx, :]
        train_y = y[train_idx]
        test_y = y[test_idx]
        ...
        res = (preds == test_y).astype(int)
        size = len(res)
        accuracies.append(np.sum(res) / size)
    return np.array(accuracies)


# if __name__ == '__main__':
#     begin_time = time.time()
#     # neg = load_docs('review_polarity/txt_sentoken/neg')
#     # pos = load_docs('review_polarity/txt_sentoken/pos')
#     # V = vocab(neg, pos)
#     # res = vectorize_docs(neg, V)
#     # model = NaiveBayes621()
#     end_time = time.time()
